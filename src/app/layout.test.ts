 ...
 *
 * `layers.css` declares `@layer base, components;` and MUST be the first
 ...
 * as the lowest cascade layer and the base reset out-ranks every component rule — the
...
describe("layout.tsx global-CSS import order", () => {
  ...
  it("imports EVERY sheet in src/styles — an orphan sheet would silently never load", () => {
    ...
    // A sheet added to the tree (e.g. a new foundation/ or semantic/ sheet) but not imported
