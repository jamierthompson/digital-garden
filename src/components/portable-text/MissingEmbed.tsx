 ...
 *
 * Keys have NO referential integrity: a saved `liveEmbed` key may point at code that's
 * gone. The resolver returns a typed `NotFound` rather than crashing the essay, and this is
 * what that `NotFound` renders — a visible, non-fatal notice in place of the widget, so the
 * rest of the essay still reads. `role="note"` (not "alert") keeps it informative, not
 * urgent. It is var-consuming only, themed by the surrounding project scope (§8).
 */
