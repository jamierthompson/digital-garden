...
/**
 * The composed Color Engine, as the entry page mounts it: the Provider frame with every slot —
 ...
 */
function renderColorEngine(slug = "demo") {
  return render(
...
describe("Color Engine (Provider + slots)", () => {
  ...
  it("stamps the live play palette onto <html> and re-stamps as the seed changes (whole-page play)", () => {
    // The ephemeral play path drives #172's `<html>` re-applier, so the WHOLE page — chrome
    // included — repaints with the generated palette. jsdom can't render the real chrome, but it
    // can prove the imperative `<html>` write happens and tracks the seed.
    renderColorEngine();
    const html = document.documentElement;
    const before = html.style.getPropertyValue("--accent");
    expect(before).not.toBe("");
    fireEvent.change(screen.getByLabelText("Seed color"), {
      target: { value: "#16a34a" },
    });
    const after = html.style.getPropertyValue("--accent");
  ...
  it("follows the viewer's preferred color scheme — no page-local toggle (#133)", () => {
    ...
    try {
      renderColorEngine();
      // Dark bg differs from light bg — both schemes are always derived, and the Color Engine paints
  ...
  it("namespaces control ids by slug so two mounts don't collide", () => {
    const { unmount } = render(
      <ColorEngineProvider slug="alpha">
        <SeedSlot />
      </ColorEngineProvider>,
    );
    expect(screen.getByLabelText("Seed color").id).toBe("ce-alpha-seed");
    unmount();
    render(
      <ColorEngineProvider slug="beta">
        <SeedSlot />
      </ColorEngineProvider>,
    );
    expect(screen.getByLabelText("Seed color").id).toBe("ce-beta-seed");
  });

  it("every slot degrades to a visible placeholder when mounted without the frame", () => {
    // A liveEmbed can be authored into ANY entry's body — a slot outside the Color Engine entry
    // must say so, not crash the essay.
    for (const [, Slot] of ALL_SLOTS) {
      const { container, unmount } = render(<Slot />);
      expect(
        within(container as HTMLElement).getByRole("note"),
      ).toHaveTextContent(/no Color Engine frame/i);
      unmount();
    }
  });
});

// Adversarial QA (QA-S13, ported to the #131 composition) for the Color Engine UI — the edges
...
function colorEngineTree(slug: string) {
  return (
    <ColorEngineProvider slug={slug}>
      <SeedSlot />
      <RulesSlot />
...
describe("QA-S13 · Color Engine UI under adversarial interaction", () => {
  it(
    ...
    () => {
      ...
      const { container } = render(
        <>
          {colorEngineTree("alpha")}
          {colorEngineTree("beta")}
        </>,
      );
      ...
      expect(document.getElementById("ce-alpha-seed")).not.toBeNull();
      expect(document.getElementById("ce-beta-seed")).not.toBeNull();
    },
  );
...
// The PAINT × TOGGLE integration seam: the page's paint (every light-dark() token, the Color Engine's
