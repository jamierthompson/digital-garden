...
// Query: *[_type == "entry" && defined(slug.current) && kind != "now"] | order(kind asc, coalesce(iterated, _createdAt) desc) {    _id,    title,    "slug": slug.current,    kind,    stage,    iterated,    summary,    "linkCount": count(coalesce(related, [])) + count(*[_type == "entry" && references(^._id)])  }
