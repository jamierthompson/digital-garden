...
3. **The OKLCH engine** (§3.2 [D3–D6, D9]) — a pure, **isomorphic** `(brandColor, scheme) → tokenSet` in `src/lib/oklch/`. It bakes literals server-side, is scheme-aware, and is **defensive — never throws** [D9]. The load-bearing, genuinely hard piece; one engine, three consumers. Read §3.2 before building against it.
