...
| **Reference-by-key**      | `keys.ts` is the single source of truth — typed resolvers `satisfies Record<Key, …>` so a missing entry is a **compile error**, returning a typed `NotFound`, never a bare lookup. **Phase 1:** authored at `src/lib/keys.ts`, dependency- and side-effect-clean. **Phase 2:** relocates to a **shared workspace package** the app and standalone Studio both consume (`[D23]` defers this move — the Studio can't import `src/*`), never duplicated. (Establish early, instantiate late `[D24]`.) | `[D10]`, `[D23]`, `[D24]`, §4.2.                                                                                                                                                 |
...
`eslint.config.mjs` defines four element types over `src/**` and enforces directional dependencies via `pnpm lint` (CI-gated). The directories are stood up empty so the rules can't rot before code arrives. **First match wins**, so specific patterns precede the `shared` catch-all. The table is the _intent_; `eslint.config.mjs` is the source of truth for the exact lint message you'll see when one fails.
...
2. **No DOM/Node globals** — `no-restricted-globals` forbids `window`/`document`/`process`/`Buffer`/… inside `src/lib/oklch/**` (full list in `eslint.config.mjs`). Imports can't catch ambient globals, so this rule does.
...
| --------------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Pure logic / utilities (no React, no I/O)           | `src/lib/*` (`breakpoints.ts`, the oklch engine)   | always — logic stays out of components                                                                                  |
