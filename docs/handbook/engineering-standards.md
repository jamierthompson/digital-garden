...
| **Reference-by-key**      | `keys.ts` is the single source of truth — it lives in the **shared workspace package** the app and Studio both consume (`[D23]`; the standalone Studio can't import `src/*`), never under `src/` and never duplicated. Type resolvers `satisfies Record<Key, …>` so a missing entry is a **compile error**; return a typed `NotFound`, never a bare lookup. | `[D10]`, `[D23]`, §4.2.                                                                                                                                                          |
