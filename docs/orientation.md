...
A personal portfolio + digital garden. Content is **entries** — notes, essays, demos, and now-updates, each an editorial article at its own flat `/[slug]` page that can mount an interactive slot. An entry that mounts one gets a **self-contained module** (`src/entries/<slug>/`) — the slot components its essay mounts and its scoped tokens — composed on a shared foundation. Content + theme seeds live in **Sanity**; the site renders on **Next.js 16 / React 19** on Vercel. The system is built out incrementally; its current state and the remaining work both live in the [GitHub issue tracker](https://github.com/jamierthompson/digital-garden/issues).
...
| Hosting       | Vercel (full SSR/RSC). Package manager: **pnpm** (workspace: app + `studio/` + `packages/oklch` + `packages/type`)                         |
...
packages/
  oklch/                   the @garden/oklch color engine — pure, isomorphic; its own workspace package so the Studio can import it too
  type/                    the @garden/type type-scale engine — pure, isomorphic; sibling of oklch (fluid clamp() + WCAG-1.4.4 zoom cap)
...
1. **Modules, not a monolith** (the Guiding principles and Entry modules sections of architecture.md). Each entry is a self-contained module under `src/entries/<slug>/`; genuinely shared parts live in plain shared `src/` modules. Dependencies point **entries → shared, never back**, and never entry → entry. This is lint-enforced (see Golden rules).

2. **Per-page engine theming, slot-scoped fonts** (the Token & theming architecture section of architecture.md). Tokens are **two tiers** — **foundation** (raw value scales at `:root`) → **semantic** (the generic role tokens components read — `--surface`, `--foreground`, `--accent`, `--font-body`, `--space-*`) — plus a **theme**: not a third tier of token names but a per-scope re-assignment of the semantic values (solved by the OKLCH engine from the authored seed — the page's own when authored, else the site default `siteSettings.theme`; there are no `--theme-*` names). The theme is **not** a CSS `@layer` — the cascade is `@layer base, components`; the override is an **unlayered `:root` `<style>`** that out-ranks the layered fallback (the "@layer trap"). Every route's `PageTheme` renders it, and React hoists it into `<head>` ahead of the chrome on hard load (flash-free), re-stamping imperatively on `<html>` on soft nav; it's inherited by all page chrome (nav, headers, prose). The `@layer` `:root` semantic color tokens are the engine's own baked **fallback** for surfaces that render un-themed (404 / error / loading). An entry's **theme fonts** are **slot-scoped**: up to three optional faces (heading / body / mono), each re-binding its role token (`--font-heading` / `--font-body` / `--font-mono`) inline on the entry's `[data-entry]` slot (`EntryScope`) — theming only that bounded interactive slot, never the page chrome; an unset role inherits the global Space Grotesk / Source Serif 4 / Geist Mono typography. The public token contract is the **semantic** layer; there are **no slug-prefixed `--<slug>-*` names** — isolation comes from scope, not a prefix.

3. **The OKLCH engine** (the OKLCH engine section of architecture.md) — a pure, **isomorphic** `(themeColor, scheme) → tokenSet` in `packages/oklch` (the `@garden/oklch` workspace package, so the Studio can import it too). It bakes literals server-side, is scheme-aware, and is **defensive — never throws**. The load-bearing, genuinely hard piece; one engine, three consumers. Read the OKLCH engine section before building against it.

4. **Reference-by-key + the `EntryScope` keystone** (the CMS ↔ code registry and Content model sections of architecture.md). Sanity stores **keys** (`componentKey`, `slotKey`, and the entry's `theme` object — `color` / `headingFont` / `bodyFont` / `monoFont`); code resolves them via typed resolvers (`keys.ts` is the single source of truth). **`EntryScope`** is the one server component that scopes a slot's **fonts**: it maps the slot's up-to-three face keys (the entry's `theme.headingFont` / `bodyFont` / `monoFont`) to inline role-token overrides (`--font-heading` / `--font-body` / `--font-mono`) on the `[data-entry]` wrapper it renders, never the page or its chrome — and an unset face emits no override, inheriting `:root`. The slot's **color** comes from the page's theme (`PageTheme`), not `EntryScope` — a `theme.color` feeds `PageTheme` (→ the page's `:root`/`<html>`), not the slot. Read the Content model section for its caching contract.
...
- **The `@layer` trap**: Next does **not** auto-assign CSS Modules to a cascade layer, and an **unlayered** module outranks **every** `@layer` style. Every `*.module.css` must declare `@layer components { … }` or stay strictly var-consuming. Enforced by `pnpm lint:css`.
