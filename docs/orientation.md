...
A personal portfolio + digital garden. Each **project is a self-contained module** — its single `/[slug]` page (the editorial article + its interactive slot), the slot components its essay mounts, and its scoped tokens — composed on a shared foundation. Content + theme seeds live in **Sanity**; the site renders on **Next.js 16 / React 19** on Vercel. The foundation, the OKLCH engine, the content model, and the first project are live; remaining work is tracked in the [GitHub issue tracker](https://github.com/jamierthompson/digital-garden/issues).
