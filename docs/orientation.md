...
4. **Reference-by-key + the `EntryScope` keystone** (the CMS ↔ code registry and Content model sections of architecture.md). Sanity stores **keys** (`componentKey`, `fontKey`, `embedKey`, `themeColor`); code resolves them via typed resolvers (`keys.ts` is the single source of truth). **`EntryScope`** is the one server component that scopes a slot's **font**: it maps the slot's `fontKey` to an inline `--font-body` on the `[data-entry]` wrapper it renders, never the page or the shell. The slot's **color** comes from the page's theme (`PageTheme`), not `EntryScope` — a `themeColor` feeds `PageTheme` (→ the page's `:root`/`<html>`), not the slot. Read the Content model section for its caching contract.
