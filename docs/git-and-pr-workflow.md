...
- **scope** (optional): the package or area, e.g. `feat(studio):`, `chore(ci):`, `fix(oklch):`.
  Useful in this four-package workspace to signal app vs. studio vs. oklch vs. type. Mind the position:
...
| 3   | `pnpm lint:routes`                                                      | `pnpm lint:routes`                       | `scripts/check-app-routes.mjs` — `src/app/` route-hygiene: only routing files under the App Router, no stray tokens/tests/config                                                                                                |
| 4   | `pnpm lint:keys`                                                        | `pnpm lint:keys`                         | `scripts/check-key-drift.mjs` — key-drift guard: runtime well-formedness + a comment-stripped `satisfies` tripwire. (The published-keys-vs-code net runs as the separate `published-keys` CI job — `pnpm lint:keys:published`.) |
| 5   | `pnpm lint:docs`                                                        | `pnpm lint:docs`                         | `scripts/check-doc-gate-sync.mjs` (gate chain identical across DoD's one command and `ci.yml`) + `check-doc-links.mjs` (markdown links/anchors) + `check-retired-citations.mjs` (retired citation schemes)                      |
| 6   | `pnpm format:check`                                                     | `pnpm format` then commit                | Prettier formatting                                                                                                                                                                                                             |
| 7   | `pnpm typecheck`                                                        | `pnpm typecheck`                         | `tsc --noEmit`                                                                                                                                                                                                                  |
| 8   | `pnpm test`                                                             | `pnpm test`                              | Vitest                                                                                                                                                                                                                          |
| 9   | `pnpm --filter studio typegen` + `git diff --exit-code sanity.types.ts` | regenerate + `git add ./sanity.types.ts` | Sanity TypeGen drift                                                                                                                                                                                                            |
| 10  | `pnpm build`                                                            | `pnpm build`                             | Next 16 / Turbopack production build                                                                                                                                                                                            |
