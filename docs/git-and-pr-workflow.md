...
| 3   | `pnpm lint:keys`                                                        | `pnpm lint:keys`                         | `scripts/check-key-drift.mjs` — key-drift guard: runtime well-formedness + a comment-stripped `satisfies` tripwire. (The published-keys-vs-code net runs as the separate `published-keys` CI job — `pnpm lint:keys:published`.) |
| 4   | `pnpm lint:docs`                                                        | `pnpm lint:docs`                         | `scripts/check-doc-gate-sync.mjs` (gate chain identical across DoD's one command and `ci.yml`) + `check-doc-links.mjs` (markdown links/anchors) + `check-retired-citations.mjs` (retired citation schemes)                      |
