...
| 3   | `pnpm lint:keys`                                                        | `pnpm lint:keys`                         | `scripts/check-key-drift.mjs` — key-drift guard: runtime well-formedness + a comment-stripped `satisfies` tripwire. (The published-keys-vs-code net runs as the sibling `published-keys` CI job — `scripts/check-published-keys.mjs` / `pnpm lint:keys:published`.) |
| 4   | `pnpm lint:docs`                                                        | `pnpm lint:docs`                         | `scripts/check-doc-gate-sync.mjs` — the gate chain stays identical across DoD's one command and `ci.yml` — plus `check-doc-links.mjs` (markdown links/anchors) and `check-retired-citations.mjs` (retired citation schemes)                                         |
