...
| [`architecture.md`](./architecture.md)                                   | **The system model** — the reference for how the system is designed: the three-layer token model (foundation → semantic → theme), the OKLCH engine, entry modules, fonts, the Sanity content model, repo & hosting, the "don't reach up" litmus. |
...
| [`working-with-agents.md`](./working-with-agents.md)                     | Cite-don't-remember, the `AGENTS.md` entry point, multi-agent orchestration, subagent briefs, clean handoffs, the independent adversarial dev↔QA loop (solo or team).                                                                            |
...
5. **[`working-with-agents.md`](./working-with-agents.md)** — when orchestrating multi-agent work.
