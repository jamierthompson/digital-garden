...
| [`architecture.md`](./architecture.md)                                   | **The system model** — the reference for how the system is designed: the three-layer token model (foundation → semantic → brand), the OKLCH engine, entry modules, fonts, the Sanity content model, repo & hosting, the "don't reach up" litmus. |
