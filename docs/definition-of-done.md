...
| Doc-gate sync | `pnpm lint:docs`                                                        | The gate chain is identical across its two declarations — this file's one command and `ci.yml` (`verify`) — plus markdown link/anchor integrity and the retired-citation guard across every tracked doc                  | Sync the divergent source (`scripts/check-doc-gate-sync.mjs`), fix the broken link/anchor (`scripts/check-doc-links.mjs`), or drop the retired citation scheme (`scripts/check-retired-citations.mjs`); the failure prints the mismatch. |
