...
| CSS layers    | `pnpm lint:css`                                                         | The **`@layer` trap** — every `*.module.css` rule must be inside `@layer` or the module stays var-consuming (an unlayered module outranks every `@layer` style)                                                          | Wrap rules in `@layer foundation\|semantic\|brand\|components`. (`scripts/check-css-layers.mjs`)                                                                                      |
...
| Doc-gate sync | `pnpm lint:docs`                                                        | The gate chain is identical across its two declarations — this file's one command and `ci.yml` (`verify`) — plus markdown link/anchor integrity across every tracked doc                                                 | Sync the divergent source (`scripts/check-doc-gate-sync.mjs`) or fix the broken link/anchor (`scripts/check-doc-links.mjs`); the failure prints the mismatch.                         |
...
values from **generic semantic tokens** (`--surface`, `--accent`, … `--font-face`, `--space-*`) and its **props**, and
