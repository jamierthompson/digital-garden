...
| Routing thin  | `pnpm lint:routes`                                                      | **`src/app/` holds only route files** + their co-located tests/private helpers + `*.module.css` / `globals.css` / assets — non-route logic, components, and token CSS keep drifting in                                                  | Move the flagged file to its `src/` home (logic → a module, styles → `src/styles/`, tests → beside their subject); the message names the file + where it goes. (`scripts/check-app-routes.mjs`) |
