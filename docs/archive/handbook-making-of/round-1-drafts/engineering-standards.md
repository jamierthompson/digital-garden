...
The coding conventions an agent applies in this repo. This is _how we write code_, not _what the system is_ — for the system model read [`../../architecture-plan.md`](../../architecture-plan.md); for binding rulings read [`../../decisions.md`](../../decisions.md) (cited here as `[D#]`).

**One rule above all others — the framework is not the one you remember.** This is Next.js **16.2.9** + React **19.2.4** with Cache Components enabled app-wide. Request APIs are async, `middleware.ts` is gone, `export const dynamic` is removed. Before you write any framework code, **read the version-matched doc** in `node_modules/next/dist/docs/` ([`../../../AGENTS.md`](../../../AGENTS.md) says the same). Don't code from training-data memory — it is stale here.

Most of what follows is **machine-enforced** (`pnpm lint · lint:css · lint:keys · typecheck · format:check · test`, all gated in CI — see [`./git-and-pr-workflow.md`](./git-and-pr-workflow.md) and [`./definition-of-done.md`](./definition-of-done.md)). Where a rule has a checker, this doc tells you the _intent_ so you stop fighting the tool.
...
| **`@/*` alias**           | Import as `@/lib/oklch/...`, never `../../../lib`.                                                                                                                            | Configured in `tsconfig.json` `paths`. Survives file moves; ESLint boundaries match on `src/**`.                                                                                 |
...
| **Reference-by-key**      | `keys.ts` is the single source of truth; type resolvers `satisfies Record<Key, …>` so a missing entry is a **compile error**; return a typed `NotFound`, never a bare lookup. | `[D10]`, §4.2.                                                                                                                                                                   |
...
`ssr: false` is **Client-Component-only** — you cannot pass it from a Server Component.
...
> ⚠️ **Async Server Components don't render in jsdom** — Vitest can't unit-test them. Test sync RSCs / Client Components with RTL; cover async RSCs and the primary flow with Playwright (Phase 3). See [`./testing.md`](./testing.md), `[D18]`.
...
> **Build-time hard error:** `Uncached data was accessed outside of <Suspense>`. Forgetting the boundary fails the build — and `pnpm build` is the last CI gate. `<Suspense>` alone does **not** make a component dynamic; it marks _where_ the dynamic hole streams.
...
Same for `headers()`, `draftMode()`, `searchParams`. Synchronous access no longer works (`…/04-functions/cookies.md`).
...
- **Vercel caveat:** in-memory cache may not persist across serverless invocations → consider `'use cache: remote'` for runtime data.
...
**`generateMetadata`** is Server-Components-only with async `params`; touching runtime data on an otherwise-prerenderable page raises an error demanding `'use cache'` or a `connection()` marker.

**Routing facts that bite:** `middleware.ts` is renamed **`proxy.ts`**, **Node runtime only** — setting `runtime` throws (`…/16-proxy.md`). `<Activity>` keeps recent routes mounted `mode="hidden"` across navigation (state + DOM preserved, effects cleaned up) — don't assume mount-on-navigate.
...
Consume tokens by their **generic public names** — `--brand-*`, `--font-face`, `--space-*` `[D2]`. `--logx-*` is a project-internal alias only; a shared cross-project embed must not depend on a project prefix.

### The `@layer` trap `[D12]` — read this

**Next does NOT auto-assign CSS Modules to a cascade layer.** An **unlayered** module's plain declarations **outrank every `@layer` style**, regardless of specificity. So:
...
`eslint.config.mjs` defines four element types over `src/**` and enforces directional dependencies via `pnpm lint` (CI-gated). The directories are stood up empty so the rules can't rot before code arrives. **First match wins**, so specific patterns precede the `shared` catch-all.

| Rule                                 | Meaning                                                                          | Message                                  |
| ------------------------------------ | -------------------------------------------------------------------------------- | ---------------------------------------- |
| `project` ⇏ other `project`          | A project module **cannot import another project** (matched by captured `slug`). | "lift shared code into a shared module." |
| `shared` ⇏ `project`                 | Dependencies point **from projects to shared, never back**.                      | —                                        |
| `oklch` ⇏ `app`/`project`/`shared`   | The engine is **self-contained**.                                                | `[D14]`                                  |
| `oklch` ⇏ `next`/`react`/`react-dom` | The engine stays **isomorphic** — no framework imports.                          | `[D14]`                                  |

**The OKLCH engine (`src/lib/oklch/**`) is isomorphic** `[D14]` — it must run identically in Node and the browser. Two guards:

1. **No framework imports** (above) — and **no `server-only`/`client-only`** (those break it; see §2).
2. **No DOM/Node globals** — `no-restricted-globals` forbids `window`, `document`, `navigator`, `location`, `localStorage`, `sessionStorage`, `history`, `process`, `Buffer`, `__dirname`, `__filename`, `global` inside `src/lib/oklch/**`. Imports can't catch ambient globals, so this rule does.
...
## 6. Quick reference — Next-16 / React-19 foot-guns
...
- [`../../decisions.md`](../../decisions.md) · [`../../architecture-plan.md`](../../architecture-plan.md) · [`../../build-phases.md`](../../build-phases.md)
