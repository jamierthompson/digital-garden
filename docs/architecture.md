...
on each control change (drag a hue, watch the palette regenerate) — is tracked in #70. No project
module ships one yet: the component registry (`COMPONENT_KEYS`) is empty until #70 lands the first
real module. A showcase module, when it lands, renders its slot's baked tokens by consuming the
scope's CSS variables; it need not call the engine at runtime (the studio is the exception — it
re-runs the pure function in JS live).

Two deliberate consequences:

- **It themes itself, on purpose.** An engine-showcase project's slot (the interactive studio,
  #70 — none ships yet) is themed like any other, so its own brand tokens are generated by the
...
without owning it — an engine-showcase module (the studio, #70) showcases the shared engine's output
rather than holding the engine (see the OKLCH engine).
