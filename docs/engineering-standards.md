...
**This is the load-bearing pattern for theming:** render the slot's `EntryScope` inside the prerendered shell, `'use cache'` it keyed on the entry's `theme.color`/`theme.headingFont`/`theme.bodyFont`/`theme.monoFont` with `cacheLife('max')` and no request APIs in that boundary — so the slot's theme `<style>` and each resolved face's `.variable` class land in the **initial static HTML** (flash-free).
