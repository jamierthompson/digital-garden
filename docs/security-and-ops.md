...
- [ ] **Deploy the Studio schema to the Content Lake.** The schema ships as part of the full Studio deploy below (`sanity deploy`, `✔ Deployed 1/1 schemas`) — after this one-time setup, the [Deploy Sanity Studio workflow](../.github/workflows/deploy-studio.yml) redeploys it automatically when `studio/**` changes on `main`. Not required for the Next build, but Presentation / Visual Editing and Sanity MCP schema validation rely on it. ⚠️ The standalone `sanity schemas deploy` is unusable: a native-thread teardown race in `@sanity/cli` SIGABRTs the process with no output, regardless of whether the deploy succeeded ([sanity-io/cli#1511](https://github.com/sanity-io/cli/issues/1511)) — the Studio deploy is the schema-refresh path until that ships fixed.
- [ ] **Deploy the Studio** (where editors sign in) — `pnpm --filter studio run deploy` (= `sanity deploy`; the `run` is required — bare `deploy` is a pnpm built-in that shadows the script); host + appId are pinned in `studio/sanity.cli.ts` so it's non-interactive. To make the hosted Studio's Presentation preview prod, deploy with `SANITY_STUDIO_PREVIEW_URL=<prod-url>` (a bare deploy reverts it to `localhost:3000`). Then point `NEXT_PUBLIC_SANITY_STUDIO_URL` at its URL.
- [ ] **Register the publish→revalidation webhook** — Sanity → API → Webhooks → Create: URL `<prod-url>/api/revalidate`, dataset `production`, trigger Create/Update/Delete, filter `_type in ["entry","siteSettings"]`, **no projection**, drafts/versions **off**, HTTP POST, **Secret = `SANITY_REVALIDATE_SECRET`** (must match Vercel exactly). Without it, a published change won't appear on prod until a redeploy. Verify: publish anything → the webhook's attempt log shows `200 {revalidated:true}`.
...
- [ ] After any **Studio schema change**: commit the regenerated root `sanity.types.ts` (CI diff-checks it). The schema redeploys itself on merge — the [Deploy Sanity Studio workflow](../.github/workflows/deploy-studio.yml) runs `sanity deploy` when `studio/**` changes on `main`.
