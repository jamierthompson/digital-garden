...
The system model is [`docs/handbook/architecture.md`](./docs/handbook/architecture.md) (refer to
...
- **Capabilities** → before hand-rolling, prefer an installed skill / subagent / MCP tool — the
  ones you need are likely already there and authed; if not, ask. Full ladder: the cite-don't-remember section of [`docs/handbook/working-with-agents.md`](./docs/handbook/working-with-agents.md).
...
- **Project rules & system model** → the handbook docs, above all
  [`docs/handbook/architecture.md`](./docs/handbook/architecture.md) (refer to its sections by
...
- **Every slice clears independent, adversarial QA before the PR.** Gate-green is
  ...
  coding agent). The lead owns the loop (see the dev↔QA loop in [`docs/handbook/working-with-agents.md`](./docs/handbook/working-with-agents.md)).
...
[the one command in `definition-of-done.md`](./docs/handbook/definition-of-done.md#1-the-one-command); CI runs the
