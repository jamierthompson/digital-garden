...
// jsdom implements neither `window.matchMedia` nor MediaQueryList, and components that mirror the
// viewer's color scheme read it through this. The stub never matches, so tests render the light
// view by default; a test that needs the dark branch stubs `matchMedia` itself (`vi.stubGlobal`)
// — unstubbing restores this default.
