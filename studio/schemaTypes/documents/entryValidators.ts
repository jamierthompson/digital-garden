 ...
 * `entry.ts` for how they attach to the `brandColor` / `brandColorDark` / `fontKey` /
 * `componentKey` fields.
 *
 ...
 * entry's COLOR: it inherits the single `/now` seed and may not set `brandColor`/`brandColorDark`
